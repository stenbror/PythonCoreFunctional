...
module TestParserExpressionRules =
    ...
    let ``Test or rule without operators``() = 
        let mutable state = ParseState.Init
        let stream = "True".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseOr(stream, &state)
        Assert.Equal([| Token.Eof(4ul, [| |]) |], rest)
        Assert.Equal(Node.True(0ul, 4ul, Token.True(0ul, 4ul, [| |] )), node)

    [<Fact>]
    let ``Test or rule with single | operator``() = 
        let mutable state = ParseState.Init
        let stream = "4 | 5".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseOr(stream, &state)
        Assert.Equal([| Token.Eof(5ul, [| |]) |], rest)
        Assert.Equal(Node.OrExpr(0ul, 5ul, 
                            Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                            Token.BitOr(2ul, 3ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                            Node.Number(4ul, 5ul, Token.Number(4ul, 5ul, [| Trivia.WhiteSpace(3ul, 4ul) |], "5" ))
                        ), node)

    [<Fact>]
    let ``Test or rule with multiple  operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 | 5 | 6".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseOr(stream, &state)
        Assert.Equal([| Token.Eof(9ul, [| |]) |], rest)
        Assert.Equal(Node.OrExpr(0ul, 9ul,
                            Node.OrExpr(0ul, 6ul, 
                                Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                                Token.BitOr(2ul, 3ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                                Node.Number(4ul, 6ul, Token.Number(4ul, 5ul, [| Trivia.WhiteSpace(3ul, 4ul) |], "5" )) ),
    ...
    [<Fact>]
    let ``Test comparison rule with single less equel operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 <= 5".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(6ul, [| |]) |], rest)
        Assert.Equal(Node.LessEqual(0ul, 6ul, 
                            Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                            Token.LessEqual(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                            Node.Number(5ul, 6ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with multiple less equal operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 <= 5 <= 6".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(11ul, [| |]) |], rest)
        Assert.Equal(Node.LessEqual(0ul, 11ul,
                            Node.LessEqual(0ul, 7ul, 
                                Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                                Token.LessEqual(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                                Node.Number(5ul, 7ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" )) ),
    ...
    [<Fact>]
    let ``Test comparison rule with single equel operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 == 5".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(6ul, [| |]) |], rest)
        Assert.Equal(Node.Equal(0ul, 6ul, 
                            Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                            Token.Equal(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                            Node.Number(5ul, 6ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with multiple equal operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 == 5 == 6".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(11ul, [| |]) |], rest)
        Assert.Equal(Node.Equal(0ul, 11ul,
                            Node.Equal(0ul, 7ul, 
                                Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                                Token.Equal(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                                Node.Number(5ul, 7ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" )) ),
                            Token.Equal(7ul, 9ul, [| Trivia.WhiteSpace(6ul, 7ul) |] ),
                            Node.Number(10ul, 11ul, Token.Number(10ul, 11ul, [| Trivia.WhiteSpace(9ul, 10ul) |], "6" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with single not equel operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 != 5".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(6ul, [| |]) |], rest)
        Assert.Equal(Node.NotEqual(0ul, 6ul, 
                            Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                            Token.NotEqual(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                            Node.Number(5ul, 6ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with multiple not equal operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 != 5 != 6".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(11ul, [| |]) |], rest)
        Assert.Equal(Node.NotEqual(0ul, 11ul,
                            Node.NotEqual(0ul, 7ul, 
                                Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                                Token.NotEqual(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                                Node.Number(5ul, 7ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" )) ),
                            Token.NotEqual(7ul, 9ul, [| Trivia.WhiteSpace(6ul, 7ul) |] ),
                            Node.Number(10ul, 11ul, Token.Number(10ul, 11ul, [| Trivia.WhiteSpace(9ul, 10ul) |], "6" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with single legacy not equel operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 <> 5".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(6ul, [| |]) |], rest)
    ...
    [<Fact>]
    let ``Test comparison rule with multiple legacy not equal operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 <> 5 <> 6".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(11ul, [| |]) |], rest)
    ...
    [<Fact>]
    let ``Test comparison rule with single in operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 in 5".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(6ul, [| |]) |], rest)
        Assert.Equal(Node.In(0ul, 6ul, 
                            Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                            Token.In(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                            Node.Number(5ul, 6ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with multiple in operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 in 5 in 6".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(11ul, [| |]) |], rest)
        Assert.Equal(Node.In(0ul, 11ul,
                            Node.In(0ul, 7ul, 
                                Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                                Token.In(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                                Node.Number(5ul, 7ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" )) ),
                            Token.In(7ul, 9ul, [| Trivia.WhiteSpace(6ul, 7ul) |] ),
                            Node.Number(10ul, 11ul, Token.Number(10ul, 11ul, [| Trivia.WhiteSpace(9ul, 10ul) |], "6" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with single is operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 is 5".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(6ul, [| |]) |], rest)
        Assert.Equal(Node.Is(0ul, 6ul, 
                            Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                            Token.Is(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                            Node.Number(5ul, 6ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" ))
                        ), node)

    [<Fact>]
    let ``Test comparison rule with multiple is operators``() = 
        let mutable state = ParseState.Init
        let stream = "4 is 5 is 6".ToCharArray() |> Tokenizer.TokenizeFromCharArray
        let node, rest = Expressions.ParseComparison(stream, &state)
        Assert.Equal([| Token.Eof(11ul, [| |]) |], rest)
        Assert.Equal(Node.Is(0ul, 11ul,
                            Node.Is(0ul, 7ul, 
                                Node.Number(0ul, 2ul, Token.Number(0ul, 1ul, [| |], "4" )),
                                Token.Is(2ul, 4ul, [| Trivia.WhiteSpace(1ul, 2ul) |] ),
                                Node.Number(5ul, 7ul, Token.Number(5ul, 6ul, [| Trivia.WhiteSpace(4ul, 5ul) |], "5" )) ),
                            Token.Is(7ul, 9ul, [| Trivia.WhiteSpace(6ul, 7ul) |] ),
                            Node.Number(10ul, 11ul, Token.Number(10ul, 11ul, [| Trivia.WhiteSpace(9ul, 10ul) |], "6" ))
                        ), node)